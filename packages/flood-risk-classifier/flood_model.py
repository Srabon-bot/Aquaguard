...
CASCADE FEATURE (2026-08-14): each horizon's model was retrained with one
extra feature, discharge_forecast_<horizon> -- the discharge-forecaster
model's OWN prediction for that same horizon, not the classifier's own past-
discharge features (those stay too; this is additional). Real A/B-tested on
held-out data (see backend/train/experiment_cascade_discharge_feature.py):
positive on PR-AUC at every horizon when each horizon gets only its own
matching forecast, not the other two horizons' -- see
models/discharge_cascade/ (a bundled, self-contained copy of the discharge-
forecaster model, per this project's "each package independently runnable"
principle -- NOT a live HTTP call to the discharge-forecaster service,
which would create a runtime dependency between packages this project has
...
class FloodRiskModel:
    ...
    def _compute_discharge_forecasts(self, features: dict, station_id: str, doy_sin: float, doy_cos: float) -> dict[str, float]:
        ...
        models' feature schemas are byte-identical (checked directly before
        this cascade design was adopted, see MODEL_BUILD_PLAN.md 2026-08-14
        -- same build_features.py pipeline, same 42 base columns, same
        ...
        if not self._discharge_models:
            raise RuntimeError(
                ...
                "which is missing. Re-copy it from backend/models/<version>-discharge-regression/."
            )
        row = dict(features)
